...
RSpec.describe PostRecord do
  describe "#last_name" do
    ...
    context "when a name is given" do
      let(:record) { PostRecord.new(officer_name: "Doe, Jane") }

      it { is_expected.to eq("Doe") }
    end

    context "when the officer's name is withheld" do
      let(:record) { PostRecord.new(officer_name: "Name Withheld") }
  ...
  describe "#first_name" do
    ...
    context "when the record has multiple given names" do
      let(:record) { PostRecord.new(officer_name: "Doe, Jane Rodham") }

      it { is_expected.to eq("Jane") }
    end

    context "when the record has a suffix" do
      let(:record) { PostRecord.new(officer_name: "Doe, John JR") }
  ...
  describe "#middle_names" do
    ...
    context "when the record has multiple given names" do
      let(:record) { PostRecord.new(officer_name: "Doe, Jane Hillary Rodham") }
    ...
    context "when the record has a middle name and a suffix" do
      let(:record) { PostRecord.new(officer_name: "Doe, John Jacob JR") }
  ...
  describe "#suffix" do
    ...
    let(:record) { PostRecord.new(officer_name: officer_name) }
  ...
  describe "#employment_start" do
    ...
    context "when a date is given" do
      let(:record) { PostRecord.new(employment_start_date: "11/5/2017") }

      it { is_expected.to eq(Date.parse("November 5, 2017")) }
    end

    context "when no date is given" do
      let(:record) { PostRecord.new(employment_start_date: "") }
  ...
  describe "#employment_end" do
    ...
    context "when a date is given" do
      let(:record) { PostRecord.new(employment_end_date: "11/5/2017") }
  ...
  describe "#name_withheld?" do
    subject { PostRecord.new(officer_name: officer_name) }
  ...
  describe "#ensure_agency" do
    ...
    let(:record) { PostRecord.new(agency: "Pinkerton") }
